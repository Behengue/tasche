<?php
	...
	try{
		if($err_name OR $err_bezeichnung)
			header('Location: ../view/typeRegistrieren.php?err_name='.$err_name.'&err_bezeichnung='.$err_bezeichnung);
		else{
			$req = $bdd->prepare('INSERT INTO type (NameType, BezeichnungType)
				VALUES(:NameType, :BezeichnungType)');
			$req->execute(array('NameType'=>$name, 'BezeichnungType'=>$bezeichnung));
